...
import { ShoppingCart, ArrowLeft, Star, Truck, Shield, Heart, LogIn } from "lucide-react";
...
const ProductDetail = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      alert(`${addedItem.produk_detail?.nama || "Product"} added to cart!`);
      navigate("/cart");
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-green-700 text-white flex justify-between items-center px-6 py-4 shadow-md">
        <div className="flex items-center gap-4">
          <Link to="/products" className="flex items-center gap-2 hover:bg-green-600 px-3 py-2 rounded">
            <ArrowLeft size={20} />
            Back to Products
          </Link>
        ...
        <div className="flex items-center gap-4">
          <Link to="/cart" className="relative p-2 hover:bg-green-600 rounded-full">
            <ShoppingCart size={22} />
          </Link>
          {!isLoggedIn ? (
            <>
              <Link to="/login" className="flex items-center gap-1 hover:text-green-200 font-medium">
                <LogIn size={18} /> Login
              </Link>
              <Link
                to="/signup"
      ...
      <main className="max-w-6xl mx-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            <img src={product.foto || ""} alt={product.nama} className="w-full h-96 object-cover" />
          </div>

          <div className="space-y-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{product.nama || "Unnamed Product"}</h1>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-gray-600">
                  {product.rating?.toFixed(1) || 0} ({product.reviews || 0} reviews)
                </span>
              </div>
            </div>

            <p className="text-gray-700 leading-relaxed">{product.deskripsi || "No description available"}</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center border rounded-lg">
                  ...
                  <button
                    onClick={() => setQuantity(Math.min(displayStok, quantity + 1))}
                    className="px-3 py-2 hover:bg-gray-100"
                  >
                    +
                  </button>
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm">Free shipping on orders over Rp200.000</span>
              </div>
              <div className="flex items-center gap-3">
